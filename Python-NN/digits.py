...
x_realTest = []
for file in files:
    x_real=keras.preprocessing.image.load_img(file,target_size=(28,28),color_mode="grayscale") #load and transform to 28x28
    x_realTest.append(keras.preprocessing.image.img_to_array(x_real)) #convert image into array
...
for i in range(25):
    ...
    pred = np.argmax(yh)
    ax.set_title('Is=' + str(y_test[i]) + ' pred=' + str(pred), fontsize=15)
plt.show()


# Predictions for my digits
for i in range(10):
    ...
    pred = np.argmax(yh)
    ax.set_title('Ireal=' + str(i) + ' pred=' + str(pred), fontsize=15)
plt.show()