...
//		arm_fir_fast_q15(&S, &dac_data[dds_phase_acc], &dac_out_q15, BLOCK_SIZE);		// Pointer type should not matter
//		dac_out = (uint16_t)dac_out_q15;
		HAL_DAC_SetValue(&hdac, DAC_CHANNEL_2, DAC_ALIGN_12B_R, (uint32_t)(dac_data[dds_phase_acc] / dds_amp_coeff));	// Removed amplitude scaling for now
	}
}
